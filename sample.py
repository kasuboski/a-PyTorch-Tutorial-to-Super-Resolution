...
if __name__ == "__main__":
    ...
    print("Beginning folder walk")
    in_folder = "../in-frames"
    out_folder = "../out-frames"
    (_, _, filenames) = next(os.walk(in_folder))

    n = 4
    all_files = [filenames[i::n] for i in range(n)]
    ...
    # first gpu has some extra allocation :shrug: and gpu 1 oom
    workers = 4
    workers = 3 if int(i) == 0 else workers
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        futures = []
        for f in all_files[int(i)]:
